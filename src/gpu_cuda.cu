...
__device__ inline float interp_u(const float *U, int n_p, int n_rho, float inv_dp,
                                 float inv_drho, float p_max, float rho_max,
                                 float p, float rho) {
...
__device__ inline float root_eq(float E, float px, float py, float pz, float j0,
                                ...
                                const float *U, int n_p, int n_rho, float inv_dp,
                                float inv_drho, float p_max, float rho_max) {
...
__device__ inline float calc_frame_energy(float px, float py, float pz, float j0,
                                          float jx, float jy, float jz, float m,
                                          const float *U, int n_p, int n_rho,
                                          float inv_dp, float inv_drho,
...
__device__ inline bool sample_jB(const float *jB, int nx, int ny, int nz,
                                 float ox, float oy, float oz, float hx, float hy,
                                 float hz, float x, float y, float z, float &j0,
...
__global__ void force_kernel(const float *rx, const float *ry, const float *rz,
                             ...
                             const float *p0, const float *meff,
                             const float *scale1, const float *scale2,
                             const float *iso3, const int *active,
                             const float *jB, const float *fi3, const float *U,
                             int n_part, int nx, int ny, int nz, int n_p,
                             int n_rho, int niter, float ox, float oy, float oz,
                             float hx, float hy, float hz, float inv_dp,
                             float inv_drho, float p_max, float rho_max, float dt,
...
bool backend_force(const ForceJob &job) {
  ...
  float *drx = up(job.rx, N), *dry = up(job.ry, N), *drz = up(job.rz, N),
        *dpx = up(job.px, N), *dpy = up(job.py, N), *dpz = up(job.pz, N),
        *dp0 = up(job.p0, N), *dm = up(job.meff, N), *ds1 = up(job.scale1, N),
        *ds2 = up(job.scale2, N), *di3 = up(job.iso3, N),
        *djB = up(job.jB, 4 * n_nodes), *dfi3 = up(job.fi3, 6 * n_nodes),
        *dU = up(job.U, (long)job.n_p * job.n_rho);
  int *dact = up(job.active, N);
  float *dnpx, *dnpy, *dnpz;
  cudaMalloc(&dnpx, N * sizeof(float));
  cudaMalloc(&dnpy, N * sizeof(float));
  cudaMalloc(&dnpz, N * sizeof(float));
  int tpb = 128, blocks = (N + tpb - 1) / tpb;
  force_kernel<<<blocks, tpb>>>(drx, dry, drz, dpx, dpy, dpz, dp0, dm, ds1, ds2,
                                di3, dact, djB, dfi3, dU, N, job.nx, job.ny,
                                job.nz, job.n_p, job.n_rho, job.niter, job.ox,
                                job.oy, job.oz, job.hx, job.hy, job.hz,
                                job.inv_dp, job.inv_drho, job.p_max, job.rho_max,
                                job.dt, dnpx, dnpy, dnpz);
  cudaError_t err = cudaDeviceSynchronize();
  bool ok = (err == cudaSuccess);
  if (ok) {
  ...
  cudaFree(drx); cudaFree(dry); cudaFree(drz); cudaFree(dpx); cudaFree(dpy);
  cudaFree(dpz); cudaFree(dp0); cudaFree(dm); cudaFree(ds1); cudaFree(ds2);
  cudaFree(di3); cudaFree(dact); cudaFree(djB); cudaFree(dfi3); cudaFree(dU);
  cudaFree(dnpx); cudaFree(dnpy); cudaFree(dnpz);
  return ok;
}
